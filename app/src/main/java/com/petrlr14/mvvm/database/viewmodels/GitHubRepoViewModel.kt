...
class GitHubRepoViewModel(private val app: Application) : AndroidViewModel(app) {
    ...
    fun retrieveRepos(user : String) = viewModelScope.launch{
        this@GitHubRepoViewModel.nuke()//primero vacio mi base

        var response = repository.retrieveRepoAsync(user).await()
        if(response.isSuccessful) with(response){
            ...
            when(this.code()){
                404->{
                    Toast.makeText(app,"No se encontró el usuario",Toast.LENGTH_LONG).show()
                }
            }
        }
    }
